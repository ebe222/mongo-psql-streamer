...
public class PostgreSqlConnector implements Connector {
    ...
    public void addConstraints(
        ...
        TableMapping tableMapping = getTableMappingOrFail(mappingName, mapping);
            ...
            List<JsonObject>  fields =  tableMapping.getFieldMappings().stream()
                .map(FieldMapping::getForeignReferrences)
                .collect(toList());

            for (JsonObject hash : fields){
                            if (hash != null) {
                                if (hash.get("active").toString().equals("true")) {
                                    log.info("add " + hash.toString());
                                    sqlExecutor.addRference(tableMapping.getDestinationName(), hash);
                                }
                            }
            }
    ...
    @Override
    public void disableTriggers(String mappingName) {
        sqlExecutor.disableAllTriggers(mappingName);
    }


    @Override
    public void enableTriggers(String mappingName) {
        sqlExecutor.enableAllTriggers(mappingName);
    }
    ...
    private List<FlattenMongoDocument> extractFlattenDocumentsFromRelatedCollection(
        ...
        if (optRelatedDocuments.get().getClass().toString().equals("class java.lang.String")){
            return Collections.emptyList();
        }
