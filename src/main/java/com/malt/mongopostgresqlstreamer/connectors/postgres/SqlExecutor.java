...
public class SqlExecutor {
    ...
    void addRference(String table, JsonObject hash) {
        setTableAsLogged(table);
        setTableAsLogged(hash.get("referrence").toString());

        log.debug("adding reference to table '{}'...", table);
        sqlExecute("DO $$ \n BEGIN \n BEGIN \n ALTER TABLE %s ADD CONSTRAINT %s UNIQUE (%s); \n EXCEPTION \n WHEN duplicate_table THEN RAISE NOTICE 'Table %s %s already exists'; \n END; \n END $$;", hash.get("referrence").toString(), hash.get("referrence_alias").toString(), hash.get("referrence_pkey").toString(),hash.get("referrence").toString(), hash.get("referrence_alias").toString());
        sqlExecute("ALTER TABLE %s ADD CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s(%s) ON DELETE CASCADE", table, hash.get("foreign_key").toString(),
         hash.get("key").toString(), hash.get("referrence").toString(), hash.get("referrence_pkey").toString());
    }
    ...
    void disableAllTriggers(String table) {
        log.debug("disabling all triggers on table ", table);
        sqlExecute("ALTER TABLE %s DISABLE TRIGGER ALL", table);
    }

    void enableAllTriggers(String table) {
        log.debug("enabling all triggers on table ", table);
        sqlExecute("ALTER TABLE %s ENABLE TRIGGER ALL", table);
    }
